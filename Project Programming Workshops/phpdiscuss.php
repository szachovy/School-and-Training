...
<h2>
    ...
    if($result = $connection->query("SELECT * FROM phptopics WHERE id = $id"))
    {
...
<form style="font-style:italic;font-weight: bold;text-align: center" method="post">
    <?php
    $fp = @fopen('phptxt/'.$id.".txt", 'r');

    # Sprawdzamy rozmiar pliku
    $size = @filesize('phptxt/'.$id.".txt");
    ...
    ?>
    <textarea name="textphp" rows="10" cols="30"></textarea>
    <br/><br/>
...
if(isset($_POST['textphp']) && isset($_POST['sumbit']) && !empty($_POST))
{
    $text = $_POST['textphp'].PHP_EOL;
    if ($fp = @fopen('phptxt/'.$id.'.txt',"a")) {
        flock($fp, 2);
        fwrite($fp, $text);
        flock($fp, 3);
        fclose($fp);
...
<a href="PHP.php"><img src="FilesCSS/img/back.png" width="350"/></a>
</body>
</html>
