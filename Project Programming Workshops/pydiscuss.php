...
<h2>
    ...
    if($result = $connection->query("SELECT * FROM pytopics WHERE id = $id"))
    {
...
<form style="font-style:italic;font-weight: bold;text-align: center" method="post">
    <?php
    $fp = @fopen('pytxt/'.$id.".txt", 'r');

    # Sprawdzamy rozmiar pliku
    $size = @filesize('pytxt/'.$id.".txt");
    ...
    ?>
    <textarea name="textpy" rows="10" cols="30"></textarea>
    <br/><br/>
...
if(isset($_POST['textpy']) && isset($_POST['sumbit']) && !empty($_POST))
{
    $text = $_POST['textpy'].PHP_EOL;
    if ($fp = @fopen('pytxt/'.$id.'.txt',"a")) {
        flock($fp, 2);
        fwrite($fp, $text);
        flock($fp, 3);
        fclose($fp);
...
<a href="Python.php"><img src="FilesCSS/img/back.png" width="350"/></a>
</body>
</html>
