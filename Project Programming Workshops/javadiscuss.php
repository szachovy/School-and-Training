...
<h2>
    ...
    if($result = $connection->query("SELECT * FROM javatopics WHERE id = $id"))
    {
...
<form style="font-style:italic;font-weight: bold;text-align: center" method="post">
    <?php
    $fp = @fopen('javatxt/'.$id.".txt", 'r');

    # Sprawdzamy rozmiar pliku
    $size = @filesize('javatxt/'.$id.".txt");
    ...
    ?>
    <textarea name="textjava" rows="10" cols="30"></textarea>
    <br/><br/>
...
if(isset($_POST['textjava']) && isset($_POST['sumbit']) && !empty($_POST))
{
    $text = $_POST['textjava'].PHP_EOL;
    if ($fp = @fopen('javatxt/'.$id.'.txt',"a")) {
        flock($fp, 2);
        fwrite($fp, $text);
        flock($fp, 3);
        fclose($fp);
        unset($_POST['textjava']);
        unset($_POST['sumbit']);
    }
}
?>
...
<a href="Java.php"><img src="FilesCSS/img/back.png" width="350"/></a>
</body>
</html>
